// De Fles — gedeelde datalaag. TV-scherm en beheerscherm lezen/schrijven
// dezelfde state in localStorage; wijzigingen in beheer verschijnen live op TV.

export const STORAGE_KEY = 'defles-state-v1';
...
// Roept cb aan bij wijzigingen uit een ander tabblad (storage event) en pollt
// als vangnet elke 2s. Retourneert een stopfunctie.
export function watchState(cb) {
  let last = localStorage.getItem(STORAGE_KEY);
  const onStorage = (e) => { if (e.key === STORAGE_KEY) { last = e.newValue; cb(loadState()); } };
  window.addEventListener('storage', onStorage);
  const iv = setInterval(() => {
    const now = localStorage.getItem(STORAGE_KEY);
    if (now !== last) { last = now; cb(loadState()); }
  }, 2000);
...
// ---- Volumio (REST API op het eigen netwerk) ----
export async function fetchVolumioState(host) {
  let h = (host || '').trim();
  if (!h) throw new Error('geen host');
  if (!/^https?:\/\//i.test(h)) h = 'http://' + h;
  h = h.replace(/\/+$/, '');
  const res = await fetch(h + '/api/v1/getState', { signal: AbortSignal.timeout(4000) });
  if (!res.ok) throw new Error('volumio http ' + res.status);
  const j = await res.json();
  return { track: j.title || '', artist: j.artist || '', playing: j.status === 'play' };
}

// ---- Spotify (PKCE-login met eigen client-id, geen secret nodig) ----
const SPOTIFY_AUTH_KEY = 'defles-spotify-auth';
...
export function spotifyConnected() {
  try {
    const a = JSON.parse(localStorage.getItem(SPOTIFY_AUTH_KEY) || 'null');
...
// Vangt de ?code= terugkeer van Spotify op; geeft true terug als verbonden.
...
async function spotifyToken() {
  let auth = null;
  try { auth = JSON.parse(localStorage.getItem(SPOTIFY_AUTH_KEY) || 'null'); } catch (e) { /* leeg */ }
